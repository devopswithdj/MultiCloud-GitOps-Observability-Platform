...
module "ResourceGroup" {
  ...
  rgname   = "${var.ProjectName}-${var.Loc}-${var.Environment}-rg-01"
...
module "AKS" {
  ...
  aks_node_rgname        = "${var.ProjectName}-${var.Loc}-${var.Environment}-aks-node-rg" # Functions in terraform substr
  aks_nodepool_name      = "${substr(var.ProjectName, 0, 3)}${var.Environment}aks"
